...
if(isset($_POST['userid']) && isset($_POST['eventid'])) {
    ...
    $check_query = "SELECT * FROM join_events WHERE user_id = $userid AND event_id = $eventid";
    $check_result = $conn->query($check_query);
    
    if($check_result->num_rows == 0) {
        // If the user is not already joined, insert into the join_events table
        $insert_query = "INSERT INTO join_events (user_id, event_id, status) VALUES ('$userid', '$eventid', '1')";
